...
add_filter( 'widget_tag_cloud_args', 'twentysixteen_widget_tag_cloud_args' );
  ...
  // Ajax Handler
  add_action('wp_ajax_fetch_posts_forhis_category', 'fetch_posts_forhis_category2');
function fetch_posts_forhis_category2() {
	// Get the Post ID from the URL
	$caytegory_id = $_REQUEST['cid'];
	
	$args = array(  'cat' => $caytegory_id ); 
...
add_action('wp_ajax_fetch_price_forhis_post', 'fetch_price_forhis_post2');
function fetch_price_forhis_post2() {
	// Get the Post ID from the URL
	$post_id = $_REQUEST['postid'];
	$cat = get_the_category($post_id);

	$value = get_field($cat[0]->slug,$post_id);
...
// update status of customr booking
add_action('wp_ajax_update_status', 'update_status2');
function update_status2() {
	// Get the Post ID from the URL
	$customer_id = $_REQUEST['customerid'];
	$st = $_REQUEST['status'];
	global $wpdb;
	$wpdb->query($wpdb->prepare("UPDATE `wp_booking_form` SET status='$st' WHERE id=$customer_id"));


	
  echo "UPDATE `wp_booking_form` SET status='$st' WHERE id=$customer_id";
 echo "Updated record";
